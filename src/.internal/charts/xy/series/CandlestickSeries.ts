...
export class CandlestickSeries extends ColumnSeries {
	 ...
	 */
	public static new(root: Root, settings: CandlestickSeries["_settings"], template?: Template<CandlestickSeries>): CandlestickSeries {
		const x = new CandlestickSeries(root, settings, true, template);
		x._afterNew();
		return x;
	}
	...
	protected _processAxisRange(axisRange: this["_axisRangeType"]) {
		...
		axisRange.columns = new ListTemplate(
			Template.new({}),
			() => Candlestick.new(this._root, { themeTags: $utils.mergeTags(axisRange.columns.template.get("themeTags", []), ["candlestick", "series", "column"]) }, axisRange.columns.template)
		);
	}
}
